  ...
  try {
    ...
    const selProjPmtRes = await pool.query(
      `SELECT COALESCE(SUM(balance_due), 0) AS total FROM invoices 
       WHERE user_id = $1 AND status != 'Paid' AND EXTRACT(MONTH FROM COALESCE(due_date, invoice_date)) = $2 AND EXTRACT(YEAR FROM COALESCE(due_date, invoice_date)) = $3`,
      [userId, month, year]
    );
    ...
    const selExpPayRes = await pool.query(
      `SELECT COALESCE(SUM(balance_due), 0) AS total FROM bills 
       WHERE user_id = $1 AND status != 'Paid' AND is_deleted = false AND EXTRACT(MONTH FROM COALESCE(due_date, bill_date)) = $2 AND EXTRACT(YEAR FROM COALESCE(due_date, bill_date)) = $3`,
      [userId, month, year]
    );
    const expected_payables = parseFloat(selExpPayRes.rows[0].total);
    ...
    const nextProjIncRes = await pool.query(
      `SELECT COALESCE(SUM(balance_due), 0) AS total FROM invoices 
       WHERE user_id = $1 AND EXTRACT(MONTH FROM COALESCE(due_date, invoice_date)) = $2 AND EXTRACT(YEAR FROM COALESCE(due_date, invoice_date)) = $3 AND status != 'Paid'`,
    ...
    const nextProjExpRes = await pool.query(
      `SELECT COALESCE(SUM(balance_due), 0) AS total FROM bills 
       WHERE user_id = $1 AND is_deleted = false AND EXTRACT(MONTH FROM COALESCE(due_date, bill_date)) = $2 AND EXTRACT(YEAR FROM COALESCE(due_date, bill_date)) = $3 AND status != 'Paid'`,
      [userId, nextMonth, nextYear]
    );
    const next_projected_expenses = parseFloat(nextProjExpRes.rows[0].total);

    const next_projected_profit = next_projected_income - next_projected_expenses;

    // 4. DETAILS
    const detRec = await pool.query(
      `SELECT i.invoice_number, c.display_name AS customer_name, COALESCE(i.due_date, i.invoice_date) AS due_date, i.balance_due, i.status 
       FROM invoices i 
       LEFT JOIN customers c ON i.customer_id = c.id 
       WHERE i.user_id = $1 AND EXTRACT(MONTH FROM COALESCE(i.due_date, i.invoice_date)) = $2 AND EXTRACT(YEAR FROM COALESCE(i.due_date, i.invoice_date)) = $3
    ...
    const detPay = await pool.query(
      `SELECT b.bill_number, v.display_name AS vendor_name, COALESCE(b.due_date, b.bill_date) AS due_date, b.balance_due, b.status 
       FROM bills b 
       LEFT JOIN vendors v ON b.vendor_id = v.id 
       WHERE b.user_id = $1 AND b.status != 'Paid' AND b.is_deleted = false AND EXTRACT(MONTH FROM COALESCE(b.due_date, b.bill_date)) = $2 AND EXTRACT(YEAR FROM COALESCE(b.due_date, b.bill_date)) = $3
