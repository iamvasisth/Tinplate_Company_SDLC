...
function CustomerStatement({ customerId }) {
  ...
  const handleDownloadPDF = () => {
    let url = `${process.env.REACT_APP_API_URL || "http://localhost:5001/api"}/customers/${customerId}/statement/pdf?token=${localStorage.getItem("token")}`;
  ...
  return (
    <div style={{ background: "#fff", padding: "30px", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
      ...
      <div className="print-hide" style={{ display: "flex", gap: "15px", marginBottom: "30px", background: "#f8fafc", padding: "15px", borderRadius: "8px", alignItems: "flex-end" }}>
        <div>
