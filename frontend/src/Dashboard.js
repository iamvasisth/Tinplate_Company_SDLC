      ...
      <section className="dash-welcome">
        <div>
          ...
          <p className="dash-welcome-sub">
            {user?.business_type ? `${user.business_type} Dashboard` : "Here is your business overview"}
          </p>
        </div>
      </section>
        ...
        <>
          ...
          <section className="dash-stats-grid">
            <div className="dash-stat-card">
              <div className="dash-stat-header">
                <div className="dash-stat-icon receivables">₹</div>
            ...
            <div className="dash-stat-card">
              <div className="dash-stat-header">
                <div className="dash-stat-icon payables">💸</div>
                <p>Total Payables</p>
              </div>
              <div className="dash-stat-content">
            ...
            <div className="dash-stat-card">
              <div className="dash-stat-header">
                <div className="dash-stat-icon income">📈</div>
                <p>Total Income</p>
              </div>
              <div className="dash-stat-content">
            ...
            <div className="dash-stat-card">
              <div className="dash-stat-header">
                <div className="dash-stat-icon expenses">📉</div>
                <p>Total Expenses</p>
              </div>
              <div className="dash-stat-content">
          ...
          <div className="zoho-dashboard-layout">
            ...
            <div className="main-charts-column">
              ...
              <div className="dash-widget-card" style={{ paddingBottom: '30px' }}>
                <h3 className="widget-title" style={{ display: 'flex', alignItems: 'center' }}>
                  <span>Monthly Overview</span>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <select 
                      value={selectedMonth} 
                      onChange={(e) => setSelectedMonth(Number(e.target.value))}
                      style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid #d1d5db', outline: 'none', background: '#f9fafb' }}
                    >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginTop: '10px' }}>
                  <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    <p style={{ margin: '0 0 5px 0', fontSize: '13px', color: '#6b7280', fontWeight: '600' }}>INCOME RECEIVED</p>
                    <h4 style={{ margin: 0, fontSize: '20px', color: '#16a34a' }}>{formatCurrency(financeData.selected_month?.income_received)}</h4>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    <p style={{ margin: '0 0 5px 0', fontSize: '13px', color: '#6b7280', fontWeight: '600' }}>TOTAL EXPENSES</p>
                    <h4 style={{ margin: 0, fontSize: '20px', color: '#dc2626' }}>{formatCurrency(financeData.selected_month?.expenses)}</h4>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    <p style={{ margin: '0 0 5px 0', fontSize: '13px', color: '#6b7280', fontWeight: '600' }}>NET PROFIT</p>
                    <h4 style={{ margin: 0, fontSize: '20px', color: (financeData.selected_month?.profit >= 0 ? '#16a34a' : '#dc2626') }}>
                      {formatCurrency(financeData.selected_month?.profit)}
                    </h4>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    <p style={{ margin: '0 0 5px 0', fontSize: '13px', color: '#6b7280', fontWeight: '600' }}>NET CASH POSITION</p>
                    <h4 style={{ margin: 0, fontSize: '20px', color: '#3b82f6' }}>{formatCurrency(financeData.selected_month?.net_cash_position)}</h4>
                  </div>
                </div>
              </div>
            ...
            <div className="side-widgets-column">
              ...
              >
                <h3 className="widget-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span>Projected Payment</span>
                  <span style={{ fontSize: '12px', color: '#3b82f6', fontWeight: '500' }}>View Details &rarr;</span>
                </h3>
                <div style={{ padding: '10px 0' }}>
                  {projectedData ? (
                    projectedData.error ? (
                      <div className="widget-empty" style={{height: '100px', color: '#dc2626'}}>
                        <p>Error: {projectedData.error}</p>
                      </div>
                    ) : (
                      <>
                        <p style={{ margin: '0 0 5px 0', fontSize: '13px', color: '#6b7280', fontWeight: '600' }}>
                        NEXT MONTH PROJECTION
                      </p>
                      <h4 style={{ margin: '0 0 15px 0', fontSize: '24px', color: '#dc2626' }}>
                      ...
                      {projectedData.bills && projectedData.bills.length > 0 ? (
                        projectedData.bills.slice(0, 3).map((bill, idx) => (
                          <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #f1f5f9', padding: '8px 0', fontSize: '13px' }}>
                            <div>
                              <p style={{ margin: 0, fontWeight: '500', color: '#334155' }}>{bill.vendor_name || 'Customer'}</p>
                              <p style={{ margin: 0, color: '#64748b' }}>{bill.bill_number}</p>
                            </div>
                        ...
                        <p style={{ fontSize: "13px", color: "#64748b" }}>No projected payments.</p>
                      )}
                    </>
                    )
                  ) : (
                    <div className="widget-empty" style={{height: '100px'}}>
