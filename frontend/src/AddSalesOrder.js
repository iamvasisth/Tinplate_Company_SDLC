/**
 * AddSalesOrder.js – New Sales Order creation form
 */
...
function AddSalesOrder() {
  ...
  if (fetching) {
    return (
      <div style={{ maxWidth: "960px", margin: "auto", padding: "30px" }}>
        <h2 style={{ marginBottom: "25px" }}>{isEditMode ? "Edit Sales Order" : "New Sales Order"}</h2>
  ...
  return (
    <div style={{ maxWidth: "960px", margin: "auto", padding: "30px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "15px", marginBottom: "15px" }}>
        <div>
          <label><strong>Sales Order Date</strong></label>
          <input type="date" value={soDate} onChange={e => setSoDate(e.target.value)} style={inputStyle} />
        </div>
        <div>
          <label><strong>Expected Shipment Date</strong></label>
          <input type="date" value={expectedShipmentDate} onChange={e => setExpectedShipmentDate(e.target.value)} style={inputStyle} />
        </div>
        <div>
      ...
      <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
        <button onClick={() => navigate(isEditMode ? `/sales-orders/${id}/document` : "/sales-orders")} style={cancelBtnStyle}>Cancel</button>
        <button onClick={handleSave} disabled={loading} style={primaryBtn}>
          {loading ? "Saving..." : (isEditMode ? "Update Sales Order" : "Save Sales Order")}
        </button>
      </div>
